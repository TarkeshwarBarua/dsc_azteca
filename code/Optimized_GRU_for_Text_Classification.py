...
# --- 2. Optimized GRU Model Architecture (Bidirectional GRU with Regularization) ---
...
def build_optimized_gru(vocab_size, embedding_dim, max_sequence_length, gru_units, dropout_rate, num_classes):
    ...
    model.add(layers.Bidirectional(layers.GRU(gru_units,
                                               return_sequences=False, # Set to True if stacking GRUs
                                               dropout=dropout_rate, # Dropout for inputs to GRU
                                               recurrent_dropout=dropout_rate # Dropout for recurrent connections
                                              )))
    model.add(layers.Dropout(dropout_rate)) # Additional dropout after GRU
...
# Build the model
model = build_optimized_gru(VOCAB_SIZE, EMBEDDING_DIM, MAX_SEQUENCE_LENGTH, GRU_UNITS, DROPOUT_RATE, NUM_CLASSES)
...
model_save_path = "optimized_gru_model.h5"
