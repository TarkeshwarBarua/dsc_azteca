...
# --- 2. Optimized LSTM Model Architecture (Bidirectional LSTM with Regularization) ---
...
#   allowing the model to capture context from both past and future elements.
# - Dropout: Regularization to prevent overfitting.
# - Recurrent Dropout: Specific to recurrent layers, regularizes the recurrent connections.
# - Adam optimizer: Adaptive learning rate optimization algorithm.

print("Building the LSTM model...")
def build_optimized_lstm(vocab_size, embedding_dim, max_sequence_length, lstm_units, dropout_rate, num_classes):
    ...
    model.add(layers.Bidirectional(layers.LSTM(lstm_units,
                                               return_sequences=False, # Set to True if stacking LSTMs
                                               dropout=dropout_rate, # Dropout for inputs to LSTM
                                               recurrent_dropout=dropout_rate # Dropout for recurrent connections
                                              )))
    model.add(layers.Dropout(dropout_rate)) # Additional dropout after LSTM
...
# Build the model
model = build_optimized_lstm(VOCAB_SIZE, EMBEDDING_DIM, MAX_SEQUENCE_LENGTH, LSTM_UNITS, DROPOUT_RATE, NUM_CLASSES)
...
# --- 4. Callbacks for Optimized Training ---
...
model_save_path = "optimized_lstm_model.h5"
